...
if (!class_exists('SLPProAdminLocationFilters')) {
    ...
    class SLPProAdminLocationFilters {
        ...
        function createstring_FilterJoinWith($field_name) {
            ...
            return
                "<select id='$field_name' name='$field_name' class='postform'>".
                    "<option value='AND' {$and_selected}>".__('and','slp-pro').'</option>'.
                    "<option value='OR'  {$or_selected} >".__('or' ,'slp-pro').'</option>'.
                '</select>'
                ;
        }
        ...
        function createstring_LocationFilterForm() {
           ...
           $HTML =
                $this->createstring_FilterInput('name',__('Store Name My Place or My* or *Place','slp-pro'),__('Name','slp-pro'), false) .
                $this->createstring_LocationPropertyDropdown( 'state' , true ) .
                $this->createstring_FilterInput('zip_filter',__('Zip Code 29464 or 294* or *464','slp-pro'),__('Zip','slp-pro')) .
        ...
        function createstring_LocationPropertyDropdown( $location_property = 'state' , $joiner = true ) {
            ...
            switch ( $location_property ) {
                case 'country':
                    $label      = __('Country','slp-pro');
                    $all_option = __('All Countries','slp-pro');
                    break;

                case 'state':
                default:
                    $label      = __('State','slp-pro');
                    $all_option = __('All States','slp-pro');
                    break;
            }
        ...
        function createstring_LocationPropertyDropdownOptions( $location_property = 'state' , $selected_value = NULL ) {
            ...
            switch ( $location_property ) {
	            ...
	            case 'country':
		            $sql_commands[] = 'select_country';
		            $sql_commands[] = 'where_valid_country';
		            $sql_commands[] = 'group_by_country';
		            $sql_commands[] = 'order_by_country';
                    break;

                case 'state':
	                $sql_commands[] = 'select_states';
	                $sql_commands[] = 'where_valid_state';
	                $sql_commands[] = 'group_by_state';
	                $sql_commands[] = 'order_by_state';
                    break;
	        ...
	        $sql_commands[] = 'limit_one';
	        $sql_commands[] = 'manual_offset';

            $HTML = '';
            $offset = 0;
	        $sql_params = array( $offset );
            while  ( $location = $this->slplus->database->get_Record( $sql_commands , $sql_params , 0 ) )  {
