...
if ( defined( 'SLPLUS_VERSION'   ) === false ) { define( 'SLPLUS_VERSION'    , '4.6.5'                             ); } // Current plugin version.
...
if ( isset( $plugin ) && ( strpos( $plugin , $slp_loader_file ) !== false ) ) {
    $slp_plugin_file = $plugin;
} elseif ( isset( $mu_plugin )  && ( strpos( $mu_plugin , $slp_loader_file ) !== false )  ) {
...
if ( defined( 'SLPLUS_FILE'      ) === false ) {
    if ( file_exists( WP_PLUGIN_DIR . '/' . basename( dirname( $slp_plugin_file ) ) . '/store-locator-le.php' ) ) {
...
if ( defined( 'SLPLUS_PLUGINDIR' ) === false ) { define( 'SLPLUS_PLUGINDIR'  , WP_PLUGIN_DIR . '/' . basename( dirname( SLPLUS_FILE ) ) . '/' ); } // Fully qualified path to this install directory.
...
//
require_once('include/class.actions.php');
$slplus_plugin->Actions = new SLPlus_Actions();

require_once('include/class.ui.php');
$slplus_plugin->UI = new SLPlus_UI();
