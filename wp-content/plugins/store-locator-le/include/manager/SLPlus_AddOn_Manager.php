...
class SLPlus_AddOn_Manager extends SLPlus_BaseClass_Object {
    ...
    private $wpdk_url    = 'https://www.storelocatorplus.com/wp-json/wp-dev-kit/v1/';
        ...
        add_action(
            'admin_notices', create_function(
                '', "echo '<div class=\"error\"><p>" .
                    __( 'Please purchase a <a href="https://www.storelocatorplus.com/product/premier-subscription/" target="store_locator_plus">Store Locator Plus Premier Subscription</a> to license your add on packs for a multisite installation. ', 'store-locator-le' ) .
                    "</p></div>';"
            )
        );
    }
    ...
    private function get_premier_license_status( $uid = null , $sid = null ) {
        ...
        if ( $license_status === false ) {
            ...
            if ( ! is_wp_error( $request ) || wp_remote_retrieve_response_code( $request ) === 200 ) {
                $license_status = $request['body'];
            } else {
                $license_status = 'invalid';
            }
            set_transient( 'slp-premier-subscription-status', $license_status, WEEK_IN_SECONDS );
        }
