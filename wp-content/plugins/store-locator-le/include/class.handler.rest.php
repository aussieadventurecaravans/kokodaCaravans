...
if ( ! class_exists('SLP_REST_Handler') ) {
    ...
    class SLP_REST_Handler extends SLPlus_BaseClass_Object {
        ...
        function initialize() {
            ...
            if ( ! defined( 'SLP_REST_SLUG'    ) ) { define( 'SLP_REST_SLUG'    , 'store-locator-plus' ) ; }
        ...
        private function setup_rest_cross_version_endpoints( $version ) {
            ...
            if ( version_compare( $version , 'v2' , '>=' ) ) {
	             ...
	             *
	             * @route   wp-json/store-locator-plus/<v2+>/option
	             * @method  WP_REST_Server::READABLE (GET)
	             *
	             * @returns WP_Error | WP_REST_Reponse
	             */
        ...
        function add_location( WP_REST_Request $request ) {
            ...
            if ( empty( $location_data ) ) {
                return new WP_Error( 'slp_missing_location_data' , $this->slplus->text_manager->get_text_string( array( 'label' , 'slp_missing_location_data' ) ) , array( 'status' => 404 ) );
            }
            ...
            if ( $result == 'not_updated' ) {
                return new WP_Error( 'slp_location_not_updated' , $this->slplus->text_manager->get_text_string( array( 'label' , 'slp_location_not_updated' ) ) , array( 'status' => 404 ) );
            }
	    ...
	    function get_smart_option( WP_REST_Request $request ) {
		    ...
		    $result = $this->slplus->smart_options->$property;
		    ...
		    $result->call_when_changed = null;
		    $result->slplus = null;
		    $result->uses_slplus = null;
