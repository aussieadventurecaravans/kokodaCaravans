...
if ( ! class_exists( 'SLP_Plugin_Style' ) ) :

/**
 * Class SLP_Plugin_Style
 *
 * @package Store Locator Plus\PluginStyle
 ...
 */
class SLP_Plugin_Style  extends SLPlus_BaseClass_Object {
    private $css_dir;
    ...
    private function create_string_addon_packs() {
        ...
        if ( ! empty( $wanted_addons ) ) {
            $html .=
                ...
                $this->slplus->text_manager->get_text_string( array( 'admin' , 'plugin_style_inactive_addons' ) ) .
                '</span>';
        ...
        if ( ! empty( $contributing_addons ) ) {
            $html .=
                '<span class="add_on_info">' .
                $this->slplus->text_manager->get_text_string( array( 'admin' , 'plugin_style_active_addons' ) ) .
                '</span>' .
    ...
    private function create_string_addon_links( $add_on_slugs ) {
        ...
        foreach ( $add_on_slugs as $add_on_slug ) {
            ...
            $html .= '<span class="product_link">' . $this->slplus->text_manager->get_web_link( 'shop_for_' . $short_slug) . '</span>';
        }
        return $html;
    }
    ...
    private function get_theme_info ($filename) {
        ...
        if ( ! empty( $filename ) ) {
            ...
            if ( ! empty( $dataBack['new_bubble'] ) && empty( $dataBack['bubble']  ) ) { $dataBack['bubble'] = $dataBack['new_bubble']; unset($dataBack['new_bubble']); }
            if ( ! empty( $dataBack['new_layout'] ) && empty( $dataBack['layout']  ) ) { $dataBack['layout'] = $dataBack['new_layout']; unset($dataBack['new_layout']); }
            if ( ! empty( $dataBack['new_result'] ) && empty( $dataBack['results'] ) ) { $dataBack['results']= $dataBack['new_result']; unset($dataBack['new_result']); }
            if ( ! empty( $dataBack['new_search'] ) && empty( $dataBack['search']  ) ) { $dataBack['search'] = $dataBack['new_search']; unset($dataBack['new_search']); }
    ...
    private function set_layout_defaults( $plugin_style_meta ) {
        ...
        foreach ( $layout_settings as $setting => $default_setting ) {
            if ( empty( $plugin_style_meta[ $setting ] ) ) {
                $plugin_style_meta[ $setting ] = $this->slplus->defaults[$default_setting];
            }
        }
