 ...
 * @property        boolean  $dataChanged            True if the location data has changed. Used to manage the
 *                  MakePersistent method, if false do not write to disk.
 ...
 * @property-read   mixed[]  $locationData           Remember the last location data array passed into set properties
 *                  via DB.
...
class SLPlus_Location {
    ...
    public function calculate_initial_distance() {
    	...
    	// Check Map Center Lat / Lng
	    //
	    if ( ! $this->is_valid_lat( $this->slplus->smart_options->map_center_lat->value ) ) {
	    	return;
	    }
	    if ( ! $this->is_valid_lng( $this->slplus->smart_options->map_center_lng->value ) ) {
		    return;
	    }

	    // Check This Location Lat / Lng
	    //
	    ...
	    $earth_radius = ( $this->slplus->smart_options->distance_unit->value === 'miles' ) ? SLPlus::earth_radius_mi : SLPlus::earth_radius_km;
	    $this->initial_distance = ( $earth_radius * acos( cos( deg2rad( $this->slplus->smart_options->map_center_lat->value ) ) * cos( deg2rad( $this->latitude ) ) * cos( deg2rad( $this->longitude ) - deg2rad( $this->slplus->smart_options->map_center_lng->value ) ) + sin( deg2rad( $this->slplus->smart_options->map_center_lat->value ) ) * sin( deg2rad( $this->latitude ) ) ) );
    }
            ...
            if ( ! $this->geocodeIssuesRendered ) {
                $errorMessage                =
                    ...
                    sprintf(
                        ...
                        $this->slplus->text_manager->get_web_link('docs_for_geocoding')
                    ) .
                    "</strong><br/>\n" .
                    $errorMessage;
    ...
    public function delete( $id = null ) {
        ...
        //
        if ( (int) $this->id < 0 ) {
            return new WP_Error( 'slp_invalid_id' , $this->slplus->text_manager->get_text_string( array( 'label' , 'slp_invalid_id' ) ) , array( 'status' => 404 ) );
        }
    ...
    public function get_location( $location_id ) {
        ...
        if ( is_null( $this->locationData ) ) {
            $result = new WP_Error( 'slp_no_such_location', $this->slplus->text_manager->get_text_string( array( 'error' , 'slp_no_such_location' ) ), array( 'status' => 404 ) );

        // Location ID does not match the one requested.
        //
        } elseif ( $location_id !== $this->id ) {
            $result = new WP_Error( 'slp_get_location_failed' , $this->slplus->text_manager->get_text_string( array( 'label' , 'slp_get_location_failed' ) ) , array( 'status' => 404 ) );
    ...
    public function is_base_field( $field_slug ) {
        $field_slug = preg_replace( '/^sl_/', '', $field_slug );

        return in_array( $field_slug, $this->dbFields );
    }
    ...
    private function set_geocoding_baseURL() {
        ...
        //
        $server_key =
            ! empty ( $this->slplus->smart_options->google_server_key->value ) ?
                '&key=' . $this->slplus->smart_options->google_server_key->value :
                '';
    ...
    function isvalid_ID( $id = null, $request_param = null ) {
        ...
        if ( ! is_null( $request_param ) && isset( $_REQUEST[ $request_param ] ) ) {
            $id = $_REQUEST[ $request_param ];
        }
     ...
     * @return bool
     */
    function is_valid_lat( $lat = null ) {
        if ( is_null( $lat ) ) { $lat = $this->latitude; }
        $regex = '/^-?([1-8]?[1-9]|[1-9]0)\.{1}\d{1,24}/';
     ...
     * @return bool
     */
    function is_valid_lng( $lng = null ) {
        if ( is_null( $lng ) ) { $lng = $this->longitude; }
        $regex = '/^-?([1]?[1-7][1-9]|[1]?[1-8][0]|[1-9]?[0-9])\.{1}\d{1,24}/';
