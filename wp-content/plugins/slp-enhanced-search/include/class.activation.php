...
if (! class_exists('SLPES_Activation')) {
	...
	class SLPES_Activation extends SLP_BaseClass_Activation {
		...
		function update() {
            ...
            // 4.4
            //
            if ( ( version_compare( $this->addon->options['installed_version'], '4.4', '<' ) ) ) {
                $optionName                                 = 'csl-slplus_find_button_label';
                $this->addon->options['label_for_find_button']  = get_option( $optionName, $this->addon->options['label_for_find_button'] );
                delete_option( $optionName );
            }
