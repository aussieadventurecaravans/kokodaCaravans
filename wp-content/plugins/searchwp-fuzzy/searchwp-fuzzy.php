...
if ( ! defined( 'SEARCHWP_FUZZY_MATCHES_VERSION' ) ) {
	define( 'SEARCHWP_FUZZY_MATCHES_VERSION', '1.4' );
}
...
class SearchWPFuzzy {
	...
	function __construct() {
		...
		add_filter( 'searchwp_term_in', array( $this, 'find_fuzzy_matches' ), 10, 3 );
	}
	...
	function find_fuzzy_matches( $terms, $engine, $original_prepped_term ) {
		...
		$missing_match = '';
		$pre_sql = $wpdb->prepare( "SELECT term FROM {$prefix}swp_terms WHERE term = %s LIMIT 1", $original_prepped_term );
		$found_term = $wpdb->get_col( $pre_sql );
		...
		if ( ! empty( $missing_match ) ) {
			...
			foreach ( $terms as $term ) {
				...
				$sql .= $wpdb->prepare( "
					( term LIKE %s
					OR reverse LIKE CONCAT(REVERSE( %s ), '%%') ", '%' . $wpdb->esc_like( $term ) . '%', $term );
			...
			<tr class="plugin-update-tr searchwp">
				<td colspan="3" class="plugin-update">
					<div class="update-message">
						<?php _e( 'SearchWP Fuzzy Matches requires SearchWP 2.4.11 or greater', 'searchwp' ); ?>
					</div>
				</td>
			</tr>
		<?php }
